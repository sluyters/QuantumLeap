 ...
 *
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, n=8: 67.06% (0.34ms)
 */
...
UVPRecognizer.prototype.shape = function(articulations) {
  ...
  // local centroid of each articulation
  let centroids = this.centroid(articulations);

  // combine all pairs of consecutive articulation's points with their local
  // centroid to create a shape
  for (let a = 0; a < this.numberOfArticulations; a += 1) {
    for (let i = 0; i < articulations[a].length - 1; i += 1) {
     // from the local centroid to the point i
     let vectorA = new Point(
       articulations[a][i].x - centroids[a].x,
       articulations[a][i].y - centroids[a].y,
       articulations[a][i].z - centroids[a].z,
     );
...
/**
 *  Compute the local centroid of each articulation.
 *
 *  @return the local centroid of each articulation.
 */
UVPRecognizer.prototype.centroid = function(articulations) {
  ...
  for (let a = 0; a < this.numberOfArticulations; a += 1) {
    ...
    centroids[a] = new Point(dX / articulations[a].length, dY / articulations[a].length, dZ / articulations[a].length);
  }
  return centroids;
}
