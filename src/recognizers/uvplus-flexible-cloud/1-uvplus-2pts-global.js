 ...
 *
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, n=8: 82.85% (0.34ms)
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, n=16: 91.60% (0.92ms)
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, n=32: 94.24% (2.32ms)
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, maxP=1, n=8: 49.68% (0.33ms)
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, maxP=2, n=8: 60.54% (0.56ms)
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, maxP=3, n=8: 65.41% (0.80ms)
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, maxP=4, n=8: 68.65% (1.00ms)
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, maxP=5, n=8: 70.44% (1.21ms)
 *
 *  If we replace the 3D-LSD (betweenShapesEuclideanDistance) with the 2D-LSD (betweenPointsEuclideanDistance)
 *  Jackknife's LeapMotion dataset, p=20, gc=8, maxT=1, n=8: 81.28% (0.28ms)
 */
...
UVPRecognizer.prototype.shape = function(articulations) {
  ...
  // combine all pairs of consecutive articulation's points with the global
  // centroid to create a shape
  for (let a = 0; a < this.numberOfArticulations; a += 1) {
    for (let i = 0; i < articulations[a].length - 1; i += 1) {
     // from the centroid to the point i
     let vectorA = new Point(
       articulations[a][i].x - centroid.x,
       articulations[a][i].y - centroid.y,
       articulations[a][i].z - centroid.z,
     );
