...
# Open results and parse json file
file = open('results-dynamic.json')
results = json.load(file)
    ...
    with open(os.path.join(datasetPath, 'testing-summary.txt'), 'w') as summaryfile:
        ...
        for recognizer in data:
            ...
            for benchmark in benchmarks:
                ...
                confusionMatrix = [ [ (elem / repetitions) * 100 for elem in line ] for line in confusionMatrix ]
                df_cm = pd.DataFrame(confusionMatrix, index = gestures, columns = gestures)
                plt.figure()
                sn.set(font_scale=1) # for label size
                sn.heatmap(df_cm, annot=True, annot_kws={"size": 8}, fmt='g', cmap='Blues', vmin=0, vmax=100)
                ...
                plt.close()
                # General data
                print('N={0}'.format(benchmark['n']))
                classAccuracies = []
                for index, line in enumerate(confusionMatrix):
                    correct = line[index]
                    accuracy = (correct/repetitions) * 100
